export interface User {
  ...
  avatarUrl?: string; role: string; provider: string; createdAt: string;
}

export interface Project {
  id: string; name: string; description: string;
  visibility: 'PRIVATE' | 'SELECTIVE' | 'PUBLIC';
  status: string; metatags: string[]; tags: string[];
  currentLevel: number; createdAt: string; updatedAt: string;
...
export interface Notification {
  ...
  read: boolean; metadata?: Record<string, string>; createdAt: string;
}

export interface Thread {
  id: string; type: string; status: string; subject: string;
...
export interface MergeParty {
  id: string; status: string; proposedEquity?: number;
  reauthAt?: string; confirmedAt?: string;
