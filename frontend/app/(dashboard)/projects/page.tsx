...
export default function NewProjectPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (tags.length < 3) {
      setError("Enter at least 3 tags (comma-separated)");
    ...
    try {
      const p = await api.post<{ id: string }>("/projects", {
        ...
        visibility: form.visibility, tags,
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-muted uppercase tracking-wider">
              Tags (min 3, max 10, comma-separated)
            </label>
            <Input
              value={form.tags}
              onChange={e => setForm(f => ({...f, tags: e.target.value}))}
              placeholder="fintech, payments, b2b, saas, latam..."
            />
            <p className="text-xs text-faint">Choose from Emoclew's tag list. Tags power project discovery and matching.</p>
          </div>
          {error && (
